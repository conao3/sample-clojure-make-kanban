...
(t/deftest auth-signup-test
  (t/is (= (res/ok "ok")
           (c.handler/handler {:name 'api.auth.signup}))))
...
(t/deftest auth-initiate-auth-test
  (t/is (= (res/ok
            {:access-token "asdfasdf"
             :id-token "asdf"
             :refresh-token "asdf"})
           (c.handler/handler {:name 'api.auth.initiate-auth}))))
...
(t/deftest auth-refresh-token-test
  (t/is (= (res/ok
            {:access-token "asdfasdf"
             :id-token "asdf"
             :refresh-token "asdf"})
           (c.handler/handler {:name 'api.auth.refresh-token}))))

(t/deftest auth-change-password-test
  (t/is (= (res/ok "ok")
           (c.handler/handler {:name 'api.auth.change-password}))))

(t/deftest auth-forgot-password-test
  (t/is (= (res/ok "ok")
           (c.handler/handler {:name 'api.auth.forgot-password}))))

(t/deftest auth-delete-user-test
  (t/is (= (res/ok "ok")
           (c.handler/handler {:name 'api.auth.delete-user}))))

(t/deftest user-get-test
  (t/is (= (res/ok {:name "asdf"})
           (c.handler/handler {:name 'api.user.get}))))

(t/deftest user-update-test
  (t/is (= (res/ok "ok")
           (c.handler/handler {:name 'api.user.update}))))
